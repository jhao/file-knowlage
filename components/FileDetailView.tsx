    ...
    <div className="flex h-[calc(100vh-64px)] overflow-hidden bg-slate-100">
      <div className="flex-1 flex flex-col border-r border-slate-200">
        <div className="h-12 bg-white border-b border-slate-200 flex items-center px-4 justify-between">
          ...
          <div className="flex items-center gap-2 text-xs"><span className="flex items-center gap-1 text-amber-500 font-medium"><ShieldAlert size={14} /> 防抓取保护开启</span></div>
        </div>
      ...
      <div className="w-[500px] bg-white flex flex-col shadow-xl z-10 border-l border-slate-200">
        <div className="p-4 border-b border-slate-200 flex justify-between items-center bg-slate-50">
          <div><h2 className="font-bold text-slate-800">档案详情</h2><p className="text-xs text-slate-500 truncate w-64" title={document.fileName}>{document.fileName}</p></div>
          {isProcessing ? <span className="text-xs text-blue-600 flex items-center gap-1 animate-pulse"><RefreshCw size={12} className="animate-spin" /> AI解析中...</span> : <button onClick={runAIAnalysis} className="text-xs flex items-center gap-1 bg-indigo-50 text-indigo-700 px-3 py-1.5 rounded hover:bg-indigo-100 border border-indigo-200"><Wand2 size={12} /> 重新 AI 解析</button>}
        </div>
